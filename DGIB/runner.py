...
class Runner(object):
    ...
    def train(self, epoch, data,u,e):
        args = self.args
        self.model.train()
        optimizer = self.optimizer

        spe_all,embeddings, ixz_loss, structure_kl_loss, consensual_loss = self.model(
            self.x,
            [
        ...
        def CLIP(hl, hr, t=1.):
            ...
            for h1,h2 in zip(hl,hr):
                ...
                loss=(0.5 * F.cross_entropy(logits, labels) + 0.5 * F.cross_entropy(logits.t(), labels))
                # print(loss)
                loss=loss/(self.len_train)
                loss.backward(retain_graph=True)
                del logits,labels
                torch.cuda.empty_cache()
                losses += loss.item()
            return losses
        infonceloss=CLIP(spe_all[:self.len_train],embeddings[:self.len_train])
        # print(infonceloss)
        alpha = 1 - args.alpha
        beta_1 = args.beta
        beta_2 = args.beta
        ...
        if infonceloss >= loss.item():
            CLIP(spe_all[:self.len_train],embeddings[:self.len_train])
        else:
            loss.backward()
        ...
        with tqdm(range(1, args.max_epoch + 1)) as bar:
            for epoch in bar:
                ...
                epoch_losses, train_auc_list, val_auc_list, test_auc_list = self.train(
                    epoch, self.data["train"],self.data['u'],self.data['e']
                )
                ...
                if average_val_auc > max_auc:
                    ...
                    test_results = self.test(epoch, self.data["test"],self.data['u'],self.data['e'])
    ...
    def test(self, epoch, data,u,e):
        ...
        test_auc_list = []
        self.model.eval()
        spe_all,embeddings, _, _, _ = self.model(
            self.x,
            [
                data["edge_index_list"][ix].long().to(args.device)
                for ix in range(self.len)
    ...
    def re_run(self):
        ...
        spe_all,embeddings, _, _, _ = self.model(
            ...
            [
             ...
             [
                self.data['u'][ix].to(args.device)
                for ix in range(self.len)
            ],
             [
                self.data['e'][ix].to(args.device)
                for ix in range(self.len)
            ])
        ...
        spe,embeddings, _, _, _ = self.model(
            self.x,
            [
        ...
        if args.feature_noise_ratio > 0:
            x_max_mean = self.data['x'].max(1)[0].mean()
            self.data['x_noise_1.0'] = self.data['x'] + torch.randn(self.data['x'].shape) * x_max_mean * 1.0
            self.data['x_noise_0.5'] = self.data['x'] + torch.randn(self.data['x'].shape) * x_max_mean * 0.5
            self.data['x_noise_1.5']=  self.data['x']+  torch.randn((self.data['x'].shape))*x_max_mean*  1.5
    ...
    def re_run_evasive(self):
        ...
        if self.args.distribution == "Bernoulli":
            filepath = (
                ...
                # + ".pth"
               "/root/myib/checkpoint/collab.pth"
            )
        ...
        self.x = x
        ss,embeddings, _, _, _ = self.model(
            self.x,
            [
                data["edge_index_list"][ix].long().to(args.device)
                for ix in range(self.len)
    ...
    def re_run_poisoning(self):
        ...
        if self.args.distribution == "Bernoulli":
            ...
            # filepath="/root/DGIB-main copy/saved_model/collab.pth"
        elif self.args.distribution == "categorical":
            filepath = (
        ...
        self.x = x
        aaa,embeddings, _, _, _ = self.model(
            self.x,
            [
                data["edge_index_list"][ix].long().to(args.device)
                for ix in range(self.len)
