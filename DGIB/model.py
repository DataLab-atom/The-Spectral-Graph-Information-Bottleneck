...
class EigenMLP(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, period):
        ...
        self.phi = nn.Sequential(nn.Linear(1, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, hidden_dim))
        self.psi = nn.Sequential(nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, 1))
...
class DGIBNN(torch.nn.Module):
    def __init__(self, args=None):
        ...
        self.args = args
        self.spe_dim=args.spe_dim
        self.spe_encoder = EigenMLP(args.spe_dim, args.hidden_dim, self.latent_size, args.period).to(self.device)
        self.init()
    ...
    def init(self):
        ...
        self.proj = nn.Sequential(nn.Linear(latent_size, latent_size), nn.PReLU(), nn.Linear(latent_size, latent_size))
    ...
    def forward(self, x_all, edge_index_all,uu,ee):
        times = len(x_all)
        sep_all=[]
        ixz, structure_kl_loss, consensual = [], [], []
            # [self.spe_encoder = spe_encoder(e,u) for range 16 n*hidden]
        for i in range(16):
            spe_dimm=self.spe_encoder(ee[i],uu[i])
            sep_all.append(spe_dimm)
